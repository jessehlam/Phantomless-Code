...
% Noise Options
% Base noise
vtsmod2 = awgn(vtsmod,137); % Add noise at SNR of ~140 (reasonable may be changed)
...
curramp3 = vts_curramp+(sin(100*ft'+2*pi*rand).*((rand/8+ft')*.1));
curramp3 = abs(vtsmod2)/vtspoly(1)+(sin(100*ft'+2*pi*rand).*((rand/8+ft')*.1));

% Use if not adding noise
curramp = vts_curramp;
...
% scatter(ft,curramp)
% hold on
% scatter(ft,abs(vtsmod2)/vtspoly(1));
% scatter(ft,curramp3);
% subplot(2,1,2)
% scatter(ft,currphi)
