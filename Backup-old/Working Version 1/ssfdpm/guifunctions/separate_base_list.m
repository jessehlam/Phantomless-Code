function [ mlist, plist, slist ] = separate_list( array )

mlist = {}; plist = {}; slist = {};
phantom = 'pfh0';    sph = 'sph';    tis = 'tis';
...
for i = 1:length( array );
    ...
    elseif ~isempty( findstr( tmp2, 'cal' ) )
        ...
        if ~isempty( findstr( tmp2, phantom ) ) || ~isempty( findstr( tmp2, 'acrin' ) ) || ~isempty( findstr( tmp2, 'ino' )) || ~isempty( findstr( tmp2, 'cavs' ))
            found = 1;
            plist( pidx ) = { tmp };
            plist2( pidx)= { tmp(1:5)};
            pidx = pidx + 1;
