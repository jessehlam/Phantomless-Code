...
function y = p1seminf(p,f,fx,nind,rho1,rho2, wt, reim_flag);
...
if length(p)==3
 	[mua, mus, Ao] = deal(p(1), p(2), p(3));
elseif length(p)==2 	%fit both mua and mus
...
%exr01 = exp(-k_josh.*r01)./r01;		%exponentials in form e(-kr)/r
%exrb1 = exp(-k_josh.*rb1)./rb1;
...
if(rho2==0),		%flag for single distance = real and imaginary
   ...
   exr02 = exp(-k_josh.*r02)./r02;
   exrb2 = exp(-k_josh.*rb2)./rb2;
